...
class _Body extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (state.hasResults) return _SearchResults(state: state);
    ...
    return CustomScrollView(
      slivers: [
        if ((tab == SearchTab.all || tab == SearchTab.articles) &&
          ...
          if (tab == SearchTab.all) _Header('Artikel', results.articles.length),
          SliverPadding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            sliver: SliverList(
        ...
        if ((tab == SearchTab.all || tab == SearchTab.users) &&
          ...
          if (tab == SearchTab.all) _Header('Pengguna', results.users.length),
          SliverPadding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
        ...
        if ((tab == SearchTab.all || tab == SearchTab.tags) &&
          ...
          if (tab == SearchTab.all) _Header('Tag', results.tags.length),
          SliverToBoxAdapter(
            child: Padding(
        ...
        if (tab == SearchTab.all && results.categories.isNotEmpty) ...[
          _Header('Kategori', results.categories.length),
          SliverToBoxAdapter(
            child: Padding(
              padding: const EdgeInsets.fromLTRB(16, 4, 16, 8),
              child: Wrap(
...
class _Header extends SliverToBoxAdapter {
  _Header(String title, int count)
    : super(
        child: Padding(
...
class _Discovery extends ConsumerWidget {
  ...
  const _Discovery({required this.onTagTap, required this.onCategoryTap});
...
class _ArticleCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final likeCount = likeCountAsync.value ?? article.likeCount;

    return GestureDetector(
      onTap: () {
        ref.invalidate(cardLikeStatusProvider((article.id, currentUserId)));
            ...
            ClipRRect(
              ...
              child: article.thumbnail != null
                  ? Image.network(
                      ...
                      fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) => _thumb(compact ? 64 : 80),
                    )
                  : _thumb(compact ? 64 : 80),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
  ...
  Widget _thumb(double size) => Container(
    width: size,
    height: size,
    decoration: BoxDecoration(
